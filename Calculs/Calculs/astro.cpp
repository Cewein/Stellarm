...
double* obliquite(double T) {
	...
	*(oblique + 2) = 21.448 - 46.8150 * T - 0.00059 * (T*T) + 0.001813 * (T*T*T);
	double longSol = longitudeSol(T);
	double longMoon = longitudeMoon(T);
	double anomalSol = anomalieSol(T);
	double anomalMoon = anomalieMoon(T);
	double longNoeudMoon = longitudeNoeudMoon(T);
...
double elongationForMoon(double T) {
	...
	elongation = elongation * (M_PI / 180);
	return elongation;
}
...
double sigmaL(double T) {
	double longMoon = longitudeMoonForMoon(T);
	double anomalSol = anomalieSolForMoon(T);
	double anomalMoon = anomalieMoonForMoon(T);
	double elongation = elongationForMoon(T);
	double distance = distanceForMoon(T);
	double A1 = a1(T);
	double A2 = a2(T);
...
double sigmaB(double T) {
	double L = longitudeMoonForMoon(T);
	double Ms = anomalieSolForMoon(T);
	double Mm = anomalieMoonForMoon(T);
	double D = elongationForMoon(T);
	double F = distanceForMoon(T);
	double A1 = a1(T);
	double A3 = a3(T);
	double E = 1 - 0.002516 * T - 0.0000074 * (T*T);
...
double longitudeEcliptique(double T) {
	double sigmal = sigmaL(T) / (M_PI / 180);
	double longMoon = longitudeMoonForMoon(T) / (M_PI / 180);
	double longitude = longMoon + sigmal / 1000000;
	return longitude;
}

double latitudeEcliptique(double T) {
	double sigmab = sigmaB(T) / (M_PI / 180);
...
double TransformInLongitudeEquatorial(double longitude, double latitude, double* oblique) {
	...
	double obli = (*oblique + (*(oblique + 1) / 100) + (*(oblique+2) / 1000)) * (M_PI / 180);
	double ascensionDroite = ((sin(longitude) * cos(obli)) - (tan(latitude) * sin(obli))) / cos(longitude);
	ascensionDroite = atan(ascensionDroite);
	ascensionDroite = ascensionDroite / (M_PI / 180);
	return ascensionDroite;
	//Put in hour
}
...
double TransformInLatitudeEquatorial(double longitude, double latitude, double* oblique) {
	...
	declinaison = declinaison / (M_PI / 180);
	return declinaison;
}
