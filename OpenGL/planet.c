...
double longitudeRealHelioPlanet(double L, double anomalieReal, double P) {
	double anomalieMoy = L - P;
	if (P > L) {
		anomalieMoy = anomalieMoy + 360;
	}
...
double longitudeHelioPlanet(double omega, double i, double argument) {
	...
	longitude = RAD2DEG(atan(longitude)) + omega;
	if (longitude > 360) {
...
void positionPlanet(MYSQL *mysql, Planet *astre, struct tm now) {
	...
	double longitude = longitudeRealHelioPlanet(*L, anomalieReal, *P);
